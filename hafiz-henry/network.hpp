...
// // Expanded Role enum for 4-party PIR + Client coordinator
// enum class Role { P0, P1, P2, P3, CLIENT };

// // NetPeer: lightweight connected peer wrapper for Boost.Asio
// struct NetPeer {
//     Role role;
//     tcp::socket sock;
...
//     // Operator << / >> overloads returning awaitable<void> for clean coroutine syntax
//     template<typename T>
//     awaitable<void> operator<<(const T& val) {
//         co_await send(val);
//         co_return;
...
// // Connection helper with retry logic (vital for async server startup)
...
struct NetPeer {
    ...
    explicit NetPeer(Role r, tcp::socket&& s) : role(r), sock(std::move(s)) {
        // Disable Nagle's Algorithm to prevent the 40ms artificial delay
