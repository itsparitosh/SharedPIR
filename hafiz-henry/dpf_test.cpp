...
// -----------------------------------------------------------------------------
// Runs three __evalinterval calls in parallel and waits for completion. (Performance helper)
// -----------------------------------------------------------------------------
...
auto gen_three_parallel(
    const prgkey_t &prgkey,   //random seed
    std::size_t nitems,       // domain size
    uint64_t target_ind,      // target index
    const leaf_t &target_value) // target value
-> std::array<std::pair<
       dpf_key<leaf_t, node_t, prgkey_t>,       //return type
...
{
    uint8_t tmp[16];
    std::memset(tmp, 0, sizeof tmp);    //lane_bytes: size of each slot
                                        // k: whch slot to fill
                                        // what nu,ber to put there
    if (lane_bytes == 1) {
        assert(k < 16);
...
// -----------------------------------------------------------------------------
// Main (take three values from command line: domain size, target index & target value)
// -----------------------------------------------------------------------------
int main(int argc, char *argv[])
{
    ...
    // }
     ...
     if (argc != 4) {
        std::cerr << argv[0] << " requires <log2(nitems)>, taregt index and target value\n";
        return 1;
    }
    ...
    uint64_t target_ind = std::stoull(argv[2]);

    using leaf_t =  __m128i; //std::array<__m128i, 2>;
    leaf_t target_value = _mm_set1_epi8(std::stoull(argv[3]));
    // leaf_t target_value = _mm_set1_epi8(10);
    // set_target_values(target_value, 100, 300);
    // target_value = _mm_set1_epi8(10);
