...
# # We redirect their logs to /dev/null so they don't clutter your screen.
# # If you need to debug a server crash, change /dev/null to 'p1.log' etc.
...
# # We DO NOT redirect output here, so you see the per-operation stats.
...
g++ -std=c++20 -O3 -march=native -fopenmp -pthread -Wno-ignored-attributes new_test.cpp -o new_test
